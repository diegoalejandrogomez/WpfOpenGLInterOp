...
void SimpleDebugObject::Render(float dt) {
	...
	GL_CHECK(glUseProgram(_shader->GetHandle()));

	//Bad way of querying uniforms
	GLuint modelLocation = glGetUniformLocation(_shader->GetHandle(), "modelMatrix");
	glUniformMatrix4fv(modelLocation,1,GL_FALSE, &glm::mat4(1.0)[0][0]);

	GLuint viewProjectionLocation = glGetUniformLocation(_shader->GetHandle(), "viewProjectionMatrix");
